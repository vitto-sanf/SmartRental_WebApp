...
function NotificaFineCorsa_AddP() {
  const [prenotazioni, setPrenotazioni] = useState()
  const [parcheggioConsegna, setParcheggioConsegna] = useState(null)
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const response = await Axios.post("http://localhost:8081/getPrenotazioniByParcheggioConsegna", { parcheggioConsegna, stato })
              ...
              <div className="modal-content">
                ...
                <div className="modal-body align-left">
                  ...
                  <div className="align-right">
                    <p className="card-text small text-muted">La corsa termina tra: {Math.floor((new Date(prenotazione.dataConsegna) - new Date()) / (1000 * 60)) + "min"}</p>
                  </div>
                </div>
                <div className="modal-footer">
                  ...
                  {Math.floor((new Date(prenotazione.dataConsegna) - new Date()) / (1000 * 60)) <= 15 ? (
                    <button onClick={() => fineCorsa(prenotazione._id)} type="button" className="btn btn-warning">
                      Fine Corsa
                    </button>
                  ) : (
            ...
            <div className="card  my-4 img-contatti">
              <div className="card-body justify-content-center">
                <div className="text-center img-contatti  pt-5 pb-3">
                  <h2 className=" mb-3">
                    <strong> Notifica fine della corsa!</strong>
                  </h2>
                  <p>Cerca la corsa indicando il parcheggio da cui inizia. Premi sul tasto "Fine corsa" per avvisare che un cliente ha appena terminato la corsa con il mezzo noleggiato. La corsa apparirà qui mezz'ora prima della sua fine</p>
                </div>
                  ...
                  <div className="col-12">
                    <div className="p-3 img-shadow mb-4 img-contatti img-border" style={{ backgroundColor: "#bcc8e9" }}>
                      <form onSubmit={handleSubmit}>
                        <select onChange={e => setParcheggioConsegna(e.target.value)} style={{ width: "auto" }} defaultValue={"DEFAULT"} className="custom-select mt-4 align-center" id="validationDefault04" required>
...
export default withRouter(NotificaFineCorsa_AddP)
